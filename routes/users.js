...
router.post('/onboarding', auth, async (req, res) => {
  ...
  try {
    ...
    const collectionMap ={
      ...
      'licensesCerts': Certification,
      'honorsAwards': HonorAward,
    };
    
    let additionalFields = [];
    ['experience', 'licensesCerts', 'honorsAwards'].forEach((field, index) => {
    ...
    if (req.body.hasOwnProperty('skills')){
      ...
      req.body.skills.forEach(async (_skill, index)=>{

        skill = new Skill({user: req.user.id, skill: _skill})
        await skill.save();
        skills.push(skill)
      })
    ...
    //now store education and respective additional fields in their collections
    
    current_user.education = education;
    //add education to education collection

    education.forEach(async (_education, idx)=>{
      _education['user']=req.user.id;
      newEducation= new Education(_education);
    ...
    //gather data from additional fields
    additionalFields.forEach(async (_field, index) => {

      //store all data in respective collections
      current=req.body[`${_field}`];
      console.log(current);

      current.forEach(async(_entity, idx)=>{
        _entity['user']=req.user.id;
        new_entity = new collectionMap[`${_field}`](_entity)
